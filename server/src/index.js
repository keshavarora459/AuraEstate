...
// Ensure MongoDB connection is established in serverless runtimes
app.use(async (req, res, next) => {
  ...
  if (mongoose.connection.readyState !== 1) {
    try {
      ...
      console.warn('Database connection check warning:', e.message);
    }
  }
  next();
});
...
if (process.env.VERCEL || process.env.VERCEL_ENV || require.main !== module) {
  console.log('Serverless environment or imported module, exporting Express app.');
} else {
  server.listen(PORT, () => {
